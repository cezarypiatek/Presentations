# Links from my presentation "How to improve software development process with Roslyn"

## Unit Testing
[RoslynNUnitLight.NetStandard](https://github.com/phoenix172/RoslynNUnitLight.NetStandard)
[Sample UT](https://github.com/cezarypiatek/MappingGenerator/tree/master/MappingGenerator/MappingGenerator/MappingGenerator.Test)

## My Visual Studio Extensions
[DDDToolbox](https://github.com/cezarypiatek/DDDToolbox)
[MappingGenerator](https://github.com/cezarypiatek/MappingGenerator)

## Tools
[Roslyn Quoter](http://roslynquoter.azurewebsites.net/)
[Explore code with the Roslyn syntax visualizer in Visual Studio](https://docs.microsoft.com/en-us/dotnet/csharp/roslyn-sdk/syntax-visualizer?tabs=csharp)

## Analyzers & VS Plugins
[DotNetAnalyzers](https://github.com/DotNetAnalyzers)
[Roslynator](https://github.com/JosefPihrt/Roslynator)
[Refactoring Essentials](http://vsrefactoringessentials.com/)
[Code Cracker](http://code-cracker.github.io/)
[Security Code Scan](https://security-code-scan.github.io/)
[Puma Scan](https://pumascan.com/)
[AnyConstraint.Analyzer](https://github.com/6A/AnyConstraint.Analyzer)
[AsyncFixer](https://marketplace.visualstudio.com/items?itemName=SemihOkur.AsyncFixer)
[Roslyn Clr Heap Allocation Analyzer](https://github.com/Microsoft/RoslynClrHeapAllocationAnalyzer)

## Others
[How Microsoft rewrote its C# compiler in C# and made it open source](https://medium.com/microsoft-open-source-stories/how-microsoft-rewrote-its-c-compiler-in-c-and-made-it-open-source-4ebed5646f98)
[Source generators](https://github.com/dotnet/roslyn/blob/master/docs/features/generators.md)
[VSCode suport](https://github.com/OmniSharp/omnisharp-vscode/issues/43)
[Learn Async](http://www.learnasync.net/)
